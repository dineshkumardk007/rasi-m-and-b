 ...
 * fire-and-forget here plus a retry drain (api/cron/drain-events) that
...
export async function logEvent(
  ...
  if (isDemo()) {
    ...
    const { data } = await supabase
      ...
      .select("id")
      .single();
    // Fire-and-forget delivery; failure leaves processed_at null for the drain.
    if (data) void deliverToN8n(data.id, type, payload);
    return;
  }
  void deliverToN8n(null, type, payload);
}
